...
# Hash categorical features handles high-cardinality columns
...
# Store FD LHS Airport Country Code for FD regularization
country_codes_train = X_train["Airport Country Code"].values
country_codes_test = X_test["Airport Country Code"].values
...
def train(model, fd_weight=0.0, epochs=25):
    ...
    for _ in range(epochs):
        ...
        if fd_weight > 0:
            loss += fd_weight * fd_loss(logits, country_codes_train)

        loss.backward()
        opt.step()

# FD violation rate metric
...
def fd_violation_rate(preds, codes):
    ...
    for _, g in dfp.groupby("code"):
        # FD violation occurs when multiple predictions appear for same code
        if g["pred"].nunique() > 1:
            violations += 1

    return violations / dfp["code"].nunique()

# EXPERIMENT: VARY FD REGULARIZATION STRENGTH
input_dim = Xtr.shape[1]
num_classes = len(le.classes_)

# Strength of FD regularization
lambdas = [0.0, 0.01, 0.05, 0.1, 0.5, 1.0, 5.0]

results = []

for lam in lambdas:
    model = FDMLP(input_dim, num_classes)
    train(model, fd_weight=lam)

    with torch.no_grad():
        preds = model(Xte).argmax(dim=1).numpy()

    acc = accuracy_score(yte, preds)
    fd_rate = fd_violation_rate(preds, country_codes_test)

    results.append({
        "lambda": lam,
        "accuracy": acc,
        "fd_violation_rate": fd_rate
    })

    print(f"λ={lam:.3f} | accuracy={acc:.3f} | FD violations={fd_rate:.3f}")
# Plot Accuracy–consistency frontier
acc = [r["accuracy"] for r in results]
viol = [r["fd_violation_rate"] for r in results]

plt.figure(figsize=(7,5))
plt.plot(viol, acc, marker="o")
plt.xlabel("FD Violation Rate ↓")
plt.ylabel("Accuracy ↑")
plt.title("Accuracy–Consistency Frontier (Experiment 3)")
plt.grid(True)
plt.show()
